##############这个代码的主要思想是BioBERT+BiLSTM+REGCN
import torch
...
class BertBiLSTM(nn.Module):
    def __init__(self, hidden_dim=256, num_layers=1, num_classes=256, freeze_bert=True):
        ...
        self.dropout = nn.Dropout(0.8)  # 添加Dropout层
...
class GCNRelationModel(nn.Module):
    def __init__(self, d_model=512, d_hidden=256, dropout=0.8):
        ...
        # 添加多头注意力层
        self.attention = MultiheadAttention(embed_dim=256, num_heads=8, dropout=0.8)
    ...
    def forward(self, input_ids, attention_mask, labels, mat):
        ...
        for i in range(fea.shape[0]):
            single_fea = fea[i]
            single_mat = mat[i]
            # print(f"GCNRelationModel single_mat shape: {single_mat.shape}")
            # print(f"GCNRelationModel single_fea shape: {single_fea.shape}")
            ...
            # 调试打印输出形状
            # print(f"Shape of outputs before permute: {outputs.shape}")

            # outputs 目前形状是 [num_nodes, hidden_dim]，我们需要增加一个伪批量维度
            outputs = outputs.unsqueeze(1)  # 变成 [num_nodes, 1, hidden_dim]

            # print(f"Shape of outputs after adding batch dimension: {outputs.shape}")

            # 在这个形状下直接应用多头注意力
            attn_output, _ = self.attention(outputs, outputs, outputs)

            # 在应用完注意力机制后, attn_output 形状会是 [num_nodes, 1, hidden_dim]
            # 因为 outputs 形状是 [num_nodes, batch_size=1, hidden_dim]

            # 转换回 (batch_size, seq_len, hidden_dim)
            attn_output = attn_output.permute(1, 0, 2)  # 从 [num_nodes, 1, hidden_dim] 到 [1, num_nodes, hidden_dim]

            # print(f"Shape of attention output after permute: {attn_output.shape}")
        ...
        # # 调试打印合并后特征的形状
        # print(f"Shape of gcn_features after concatenation: {gcn_features.shape}")
        ...
        # # 调试打印展平后的特征形状
        # print(f"Shape of gcn_features after flattening: {gcn_features.shape}")
...
def train_model(model, train_data_loader, optimizer, criterion, device):
    ...
    for batch in train_data_loader:
        ...
        predicted_probs.extend(F.softmax(outputs, dim=1).detach().cpu().numpy())  # Use detach() here
